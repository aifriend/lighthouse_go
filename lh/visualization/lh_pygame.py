...
from lh.config.configuration import P_NAME_IDX, A_TYPE_IDX, d_a_color, d_type_rev, MONEY_IDX, TIME_IDX, CARRY_IDX, \
    HEALTH_IDX
...
def init_visuals(world_width: int, world_height: int, verbose=True) -> Optional[Tuple[Any, Any, Any]]:
    ...
    if verbose:
        ...
        pygame.display.set_caption('LH visualization Python game')
...
def update_graphics(board: np.ndarray, arena, screen, clock, fps: int = 1) -> bool:
    ...
    :param arena: canvas to draw game state upon
    :param screen: canvas to draw game state upon
