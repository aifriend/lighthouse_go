...
class Board:
    ...
    def execute_move(self, move, color):
        ...
        if self._isLegalMove(pieceno, x2, y2):
    ...
    def _isLegalMove(self, pieceno, x2, y2):
        try:
            ...
            piecetype = piece[2]
            if (piecetype == -1 and self.time % 2 == 0) or (
                    piecetype != -1 and self.time % 2 == 1): return -5  # wrong player

            for item in self.board:
                if item[0] == x2 and item[1] == y2 and item[2] > 0:
                    if piecetype != 2: return -10  # forbidden space
            for apiece in self.pieces:
                if y1 == y2 and y1 == apiece[1] and ((x1 < apiece[0] and x2 >= apiece[0]) or (
                        x1 > apiece[0] and x2 <= apiece[0])): return -20  # interposing piece
                if x1 == x2 and x1 == apiece[0] and ((y1 < apiece[1] and y2 >= apiece[1]) or (
                        y1 > apiece[1] and y2 <= apiece[1])): return -20  # interposing piece
